...
toggle_systemd(){
	[[ ! -f /etc/wsl.conf ]] && exit 1
	local status_wsl_systemd=$(grep -Po "(?<=systemd=).+" /etc/wsl.conf)
	if [[ $status_wsl_systemd = 'true' ]]; then
		sudo perl -pi -e "s/(?<=systemd=).+/false/" /etc/wsl.conf  
		echo "Disabled systemd"
	elif [[ -n $status_wsl_systemd ]]; then
		sudo perl -pi -e "s/(?<=systemd=).+/true/" /etc/wsl.conf  
		echo "Enabled systemd"
	else
		sudo bash -c 'echo "systemd=true" >> /etc/wsl.conf'  
		echo "Systemd not present, now enabled"
	fi
}
